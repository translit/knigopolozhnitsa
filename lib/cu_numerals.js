...
// Strip combining diacritical marks (titlo, accents, etc.)
...
// Returns true if codepoint is a combining mark (for accumulation in numeral strings).
...
// Algorithm: strip combining marks, sum the values of all known base letters.
// Handles the thousands marker ҂ (U+0482): each ҂ before a letter multiplies
// that letter's value by 1000^n where n is the count of consecutive ҂ signs.
// Example: ҂з = 7000, ҂зфл҃г = 7533.
// Note: the rare space-separated тьмы form (>10000 with a space) is not supported.
// Works because CU numerals use additive notation regardless of letter order.
...
// Returns true if the string consists entirely of CU numeral base letters and
// combining marks — i.e., it is a bare verse number, not text.
// By default permissive (any combination of numeral letters passes), which is
// robust for noisy OCR-derived source. Pass { strict: true } to require
// canonical numeral structure (uses isCanonicalNumeral internally).
function isVerse(content, { strict = false } = {}) {
  const str = content.normalize('NFC').trim();
  if (!str) return false;
  if (strict) return isCanonicalNumeral(str);
...
if (require.main === module) {
  ...
  // isVerse: default permissive, strict opt-in
  check('isVerse гл҃а default = true', isVerse('гл҃а'), true);              // false positive, permissive
  check('isVerse гл҃а strict = false', isVerse('гл҃а', {strict:true}), false); // canonical rejects it
  check('isVerse а strict = true',     isVerse('а', {strict:true}), true);   // valid numeral
  check('isVerse а default = true',    isVerse('а'), true);
